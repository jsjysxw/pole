...
import cn.com.avatek.pole.drawmap.models.ForTreeItem;
...
public class RightTreeLayoutManager implements TreeLayoutManager {
    ...
    private void doNext(int msg, NodeModel<String> next, TreeView treeView) {
        ...
        sLayout(treeView, (NodeView) view);
...
//        }
     ...
     * @param rootView
     */
    private void sLayout(TreeView treeView, NodeView rootView) {
        ...
        if (treeNode != null) {
            ...
            if (size == 0) {
                ...
                for (int i = size - 1; i >= 0; i--) {
                    NodeView midChildNodeView = (NodeView) treeView.findNodeViewFromNodeModel(childNodes.get(i));
                    ...
                    if(midChildNodeView.getTreeNode().coorl!=0&&midChildNodeView.getTreeNode().coort!=0) {
                        midChildNodeView.layout(midChildNodeView.getTreeNode().coorl, midChildNodeView.getTreeNode().coort,
                                midChildNodeView.getTreeNode().coorl+midChildNodeView.getMeasuredWidth(), midChildNodeView.getTreeNode().coort+midChildNodeView.getMeasuredHeight());
                    }else {
                        midChildNodeView.layout(left, top, right, bottom);
                        midChildNodeView.getTreeNode().setCoorl(left);
                        midChildNodeView.getTreeNode().setCoort(top);
                        midChildNodeView.getTreeNode().setCoorr(right);
                        midChildNodeView.getTreeNode().setCoorb(bottom);
                    }
                }
            }
...
//            }
    ...
    private void rootTreeViewLayout(NodeView rootView) {
        ...
        if(rootView.getTreeNode().coorl!=0&&rootView.getTreeNode().coort!=0&&rootView.getTreeNode().coorr!=0&&rootView.getTreeNode().coorb!=0) {
            ...
            rootView.getTreeNode().setCoorl(lr);
            rootView.getTreeNode().setCoort(tr);
            rootView.getTreeNode().setCoorr(rr);
            rootView.getTreeNode().setCoorb(br);
        }
    }
}
