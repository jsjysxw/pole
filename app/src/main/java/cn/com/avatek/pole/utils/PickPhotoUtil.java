...
public class PickPhotoUtil {
    ...
    public interface PickPhotoCode {
         ...
         * 相片选中标记
         */
        public static final int PHOTO_SELECT = BASE_CAMERA + 3;
    }
    ...
    public void localPhoto(Activity mActivity) {
        if (isStorageState()) {
            ...
            mActivity.startActivityForResult(mIntent, PickPhotoCode.PICKPHOTO_LOCAL);
        } else {
            Log.e(TAG, "未找到SD卡");
    ...
    public String takePhoto(Activity mActivity, String userName, String path) {
        if (isStorageState()) {
            File photoDir = new File(CAMERA_PATH);// 图片的存储目录
            if (!photoDir.exists()) {
                photoDir.mkdirs();
            }
            //File imgFile = new File(CAMERA_PATH + "/" + path);
            ...
            if (null != imgFile) {
                ...
                // 调用系统拍照
                mActivity.startActivityForResult(mIntent, PickPhotoCode.PICKPHOTO_TAKE);
    ...
    private String insert(Context mContext, Object obj) {
        ...
        try {
            ...
            insertImageUri = MediaStore.Images.Media.insertImage(mContext.getContentResolver(), bitmap, fileName, fileName);
